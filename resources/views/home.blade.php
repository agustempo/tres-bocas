                ...
                <div class="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                    <a href="{{ route('home') }}"
                       class="shrink-0 px-4 py-2 rounded-full text-sm font-medium border transition-colors duration-150
                              {{ !$catSlug && !$search ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-400' }}">
                        {{ __('ui.categories') }} &mdash; {{ __('ui.home') }}
                    </a>
                    @foreach ($categories as $cat)
                        <a href="{{ route('home', ['category' => $cat->slug]) }}"
                           class="shrink-0 px-4 py-2 rounded-full text-sm font-medium border transition-colors duration-150
                                  {{ $catSlug === $cat->slug ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-400' }}">
                            {{ $cat->name }}
                        </a>
                    @endforeach
                </div>
                ...
                <div class="py-8">
                    @if ($listings->isEmpty())
                        <div class="text-center py-20">
                            ...
                            <p class="text-gray-500 text-lg font-medium">{{ __('ui.no_results') }}</p>
                    ...
                    @else
                        <p class="text-sm text-gray-400 mb-6">
                            ...
                            @if ($search) — "<span class="text-gray-700 font-medium">{{ $search }}</span>" @endif
                        </p>
